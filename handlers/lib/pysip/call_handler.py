...
class CallHandler:
    ...
    async def say_pre_generated(self, audio_bytes: io.BytesIO):
        """Play pre-generated audio bytes without TTS delay"""
        try:
            app_stopped_task = asyncio.create_task(self.call._wait_stopped())
            
            # Check if call is still active
            if app_stopped_task.done():
                raise RuntimeError("Call is no longer ongoing")
            
            # Use the pre-generated audio bytes directly
