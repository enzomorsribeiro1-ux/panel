...
class SipAccount:
     ...
     def __init__(
          ...
          *,
          ...
          self.hostname, self.port = self.__parse_hostname(
               hostname,
               connection_type
          )
     ...
     async def _get_connection_type(self):
          ...
          self.port = connection_ports.get(
               con_type[0]
          )
          self.hostname = self.hostname.split(":")[0] + ":" + str(self.port)
          logger.log(
               logging.INFO,
               "Connection type detected: %s",
               con_type[0]
          )
          return con_type[0]

     async def register(self):
          # If already registered, return the current status
          if self.__sip_client and self.__client_task and not self.__client_task.done():
               return await self.__sip_client.registered
          ...
          for callback in self.__pending_callbacks:
               self.__sip_client._register_callback(
                    "incoming_call_cb",
                    callback
               )
          self.__pending_callbacks = []

          self.__client_task = asyncio.create_task(
               self.__sip_client.run()
          )
